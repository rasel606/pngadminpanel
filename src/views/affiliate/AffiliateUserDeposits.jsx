...
const AffiliateWithdrawManagement = () => {
  ...
  const fetchWithdrawals = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/affiliate/management/withdrawals${query}`, {
        method: 'GET',
        headers: {
  ...
  const handleApproveWithdrawal = async (id) => {
    try {
      const token = localStorage.getItem('admin_auth_token')
      const response = await fetch(`/api/admin/affiliate/management/withdrawals/${id}/approve`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRejectWithdrawal = async (id) => {
    try {
      const token = localStorage.getItem('admin_auth_token')
      const response = await fetch(`/api/admin/affiliate/management/withdrawals/${id}/reject`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const filteredWithdrawals = withdrawals.filter(
    (withdrawal) =>
      withdrawal.affiliateId?.toString().includes(searchTerm) ||
      withdrawal.affiliateName?.toLowerCase().includes(searchTerm.toLowerCase()),
  )
    ...
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Affiliate Withdrawals Management</strong>
          </CCardHeader>
          <CCardBody>
              ...
              <CTable hover responsive>
                <CTableHead>
                  <CTableRow>
                    ...
                    <CTableHeaderCell scope="col">Bank Account</CTableHeaderCell>
                  ...
                  {filteredWithdrawals.length > 0 ? (
                    filteredWithdrawals.map((withdrawal) => (
                      <CTableRow key={withdrawal._id}>
                        <CTableDataCell>{withdrawal.affiliateId}</CTableDataCell>
                        <CTableDataCell>{withdrawal.affiliateName}</CTableDataCell>
                        <CTableDataCell>${withdrawal.amount}</CTableDataCell>
                        <CTableDataCell>{withdrawal.bankAccount?.name}</CTableDataCell>
                        <CTableDataCell>
                          <span
      ...
      <CModal visible={showModal} onClose={() => setShowModal(false)} size="lg">
        ...
        <CModalBody>
          {selectedWithdrawal && (
            <div>
              <p>
                <strong>Affiliate:</strong> {selectedWithdrawal.affiliateName}
              </p>
              <p>
              ...
              <p>
                <strong>Bank Account:</strong> {selectedWithdrawal.bankAccount?.name}
              </p>
              <p>
                <strong>Account Number:</strong> {selectedWithdrawal.bankAccount?.accountNumber}
              </p>
              <p>
...
export default AffiliateWithdrawManagement

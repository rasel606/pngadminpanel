...
const Table = ({ data, loading, error, onStatusUpdate }) => {
  const columns = [
    {
      title: 'Amount',
      key: 'amount',
    ...
    },
    {
      ...
      key: 'status',
      render: (row) => (
        <StatusBadge status={row.status} onUpdate={() => handleQuickStatusUpdate(row)} />
      ),
    },
    {
      title: 'User ID',
  ...
  const handleQuickStatusUpdate = (row) => {
    // Your status update logic
  }

  const handleRowAction = (row) => {
    // Your row action logic (like opening a modal)
  }
